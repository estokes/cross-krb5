...
//! use cross_krb5::{ClientCtx, ServerCtx, K5Ctx, K5ServerCtx, InitiateFlags, AcceptFlags};
//!
//! // make a pending context and a token to connect to `service/host@REALM`
//! let (pending, token) = ClientCtx::initiate(InitiateFlags::empty(), None, "service/host@REALM")?;
//!
...
//! let (mut server, token) = ServerCtx::accept(AcceptFlags::empty(), Some("service/host@REALM"), &*token)?;
//!
